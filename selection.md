...
|  |  |  |  |  |  |  |  |
| **-** |   минимум безопасности   |   централизованная структура   |   нет pub/sub   |   слишком тяжелый для IoT-устройств   |    сложность реализации    |  сложное устройство   |   тяжелый для IoT-устройств   |
...
* **Модель (центр, распредеелнная)**
  * центр — MQTT;
  * распределенная — CoAP, либо DDS (для гибкой настройки IoT-системы)
* **Совместимость с АО**
  * MQTT;
  * CoAP;
  * HTTP (если нет особых ограничений по ресурсам);
  * OPC UA (сложность настройки и запуска, высокая стоимость конечной системы);
